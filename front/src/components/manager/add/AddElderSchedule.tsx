...
interface AddElderScheduleProps {
  ...
  selectedTwo: number;
  setSelectedTwo: React.Dispatch<React.SetStateAction<number>>;
}
  ...
  const handleGauge1 = () => {
    if (selectedTwo < maxSteps) {
      if (
        ...
        setSelectedTwo((prev: number) => prev + 1);
      }
    }
  };
      ...
      {selectedTwo == 0 ? (
        ...
        <>
          ...
          <div className="w-[682px] h-[123px] bg-[#F7F8FA] rounded-[14px] mt-[30px] flex pl-[23px]">
            <p className="text-[22px] text-[#191A1C] font-[600] mt-[30px]">
              시급
            </p>
            <input
              value={formatCurrency(salaryWeek)}
            ...
            <p className="text-[26px] font-[600] text-[#909090] ml-[179px] mt-[74px]">
              월급{" "}
          ...
          <div className="flex w-[726px] justify-between relative">
            <button
              onClick={() => setSelectedTwo((prev: number) => prev - 1)}
